...
public:
	...
	Vector<Moveable*> GenerateMaze(Player* player, const Coord& start = Coord(2, 2))
	{
		...
		while (!stack.Empty())
		{
			...
			{
				Coord position = current + offsets[i];

				if (position.GetY() > 1 && position.GetY() < height - 1 &&
					position.GetX() > 1 && position.GetX() < width - 1 &&
					dynamic_cast<Wall*>(maze[position.GetY()][position.GetX()].content.get()))
				{
					SetPassage(current, position);
					...
					if (objects.Size() > 0)
					{
						...
						if (rnd < objects.Size())
						{
							...
							maze[position.GetY()][position.GetX()].content = objects[rnd];
							objects.Remove(rnd);
	...
	Entity* InentifyObject(const Coord coord) const
	{
